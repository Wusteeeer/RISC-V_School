...
void LCD_DeleteChar(u16 *x, u16 *y, u16 back_color){

	if(*x + *y != 0){ //If not on first position

		if(*x == 0){ //If on start of row
		...
		LCD_Address_Set(*x, *y, (*x) + 8 - 1, (*y) + 16 - 1); //Move cursor to removed character
